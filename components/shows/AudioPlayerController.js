...
function AudioPlayerController({position,duration,bufferedPosition,error,play,currentTrack}){
    ...
    useEffect(()=>{
        ...
        let fetchRes = fetch(likeShowApi+'/'+currentTrack.slot_uid+'?action='+liked,options);
        fetchRes.then(res =>{
            console.log(res)
            return res.json()})
        .then(res=>{
        if(!res.err){
            console.log('like show success')
        }else{
            console.log('like show failed')
        }
        }).catch(err=>{
            console.log(err)
